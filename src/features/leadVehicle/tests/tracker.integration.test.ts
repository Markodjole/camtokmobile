...
function vehicleAt(
  ...
  return { vehicleType: "vehicle", confidence, boundingBox: box };
}

describe("LeadVehicleTracker + pipeline integration", () => {
  it("acquires a persistent corridor vehicle", async () => {
    ...
    const frames = [
      [],
      [vehicleAt({ x: 0.46, y: 0.42, width: 0.12, height: 0.15 })],
      [vehicleAt({ x: 0.47, y: 0.43, width: 0.13, height: 0.16 })],
      [vehicleAt({ x: 0.48, y: 0.44, width: 0.14, height: 0.17 })],
      [vehicleAt({ x: 0.48, y: 0.44, width: 0.14, height: 0.17 })],
      [vehicleAt({ x: 0.48, y: 0.44, width: 0.14, height: 0.17 })],
      [vehicleAt({ x: 0.48, y: 0.44, width: 0.14, height: 0.17 })],
    ];
    ...
    if (snap.status === "tracking") {
      expect(snap.leadVehicle?.vehicleType).toBe("vehicle");
    }
    await pipeline.stop();
  });

  it("tracks two separate vehicles in frame", () => {
    resetTrackIdSequence();
    ...
    for (let i = 0; i < 5; i += 1) {
      ...
      tracker.update(
        [
          vehicleAt({ x: 0.78, y: 0.55, width: 0.2, height: 0.28 }, 0.95),
          vehicleAt({ x: 0.46, y: 0.38, width: 0.1, height: 0.16 }, 0.8),
        ],
        t,
      );
    }
    ...
    expect(mature.every((m) => m.vehicleType === "vehicle")).toBe(true);
  });
